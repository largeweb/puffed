...
export async function GET(request: Request) {
  ...
  // 4. Unique brands
  const uniqueBrands = new Set(checkins.map((c: { brand: string }) => c.brand)).size;
  bests.push({
  ...
  // 5. Average rating
  const ratedCheckins = checkins.filter((c: { rating?: number }) => c.rating);
  if (ratedCheckins.length > 0) {
    const avgRating = ratedCheckins.reduce((sum: number, c: { rating: number }) => sum + c.rating, 0) / ratedCheckins.length;
    bests.push({
  ...
  // 6. Five star count
  const fiveStarCount = checkins.filter((c: { rating?: number }) => c.rating === 5).length;
  if (fiveStarCount > 0) {
    bests.push({
  ...
  const mostLiked = checkins.reduce((max: { like_count: number; brand: string } | null, c: { like_count: number; brand: string }) => 
  ...
  const mostCommented = checkins.reduce((max: { comment_count: number; brand: string } | null, c: { comment_count: number; brand: string }) => 
  ...
  for (const c of checkins) {
    const day = new Date((c as { created_at: number }).created_at * 1000).toDateString();
  ...
  for (const c of checkins) {
    const brand = (c as { brand: string }).brand;
  ...
  const uniqueDays = [...new Set(checkins.map((c: { created_at: number }) => {
  ...
  let maxStreak = 1;
  let currentStreak = 1;
  let streakEndDate = uniqueDays[0];
  ...
  for (const c of checkins) {
    const hour = new Date((c as { created_at: number }).created_at * 1000).getHours();
  ...
  const nightSmokes = checkins.filter((c: { created_at: number }) => {
    const h = new Date(c.created_at * 1000).getHours();
    return h >= 22 || h < 4;
  }).length;
  const morningSmokes = checkins.filter((c: { created_at: number }) => {
    const h = new Date(c.created_at * 1000).getHours();
    return h >= 4 && h < 10;
  }).length;
  ...
  // 14. Photos uploaded
  const photosCount = checkins.filter((c: { image_url?: string }) => c.image_url).length;
  if (photosCount > 0) {
  ...
  // 15. Mood variety
  const uniqueMoods = new Set(checkins.filter((c: { mood?: string }) => c.mood).map((c: { mood: string }) => c.mood)).size;
  if (uniqueMoods >= 3) {
    bests.push({
      label: "Mood Range",
